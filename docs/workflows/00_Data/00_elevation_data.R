...
file_names <- file.path("data", "Raw", paste0(c(
...
# DTM
ville_vancouver_dtm <- list.files(file.path("data", "Raw"), pattern = "ville_vancouver_dtm", full.names = TRUE) %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

ville_burnaby_dtm <- list.files(file.path("data", "Raw"), pattern = "ville_burnaby_dtm", full.names = TRUE) %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

ville_surrey_dtm <- list.files(file.path("data", "Raw"), pattern = "ville_surrey_dtm", full.names = TRUE) %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

dtm_list <- list(ville_vancouver_dtm, ville_burnaby_dtm, ville_surrey_dtm)
dtm_list$filename <- file.path("data", "DTM_Vancouver_1m.tif")
...
# DSM
ville_vancouver_dsm <- list.files(file.path("data", "Raw"), pattern = "ville_vancouver_dsm", full.names = TRUE) %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

ville_burnaby_dsm <- list.files(file.path("data", "Raw"), pattern = "ville_burnaby_dsm", full.names = TRUE) %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

ville_surrey_dsm <- list.files(file.path("data", "Raw"), pattern = "ville_surrey_dsm", full.names = TRUE)[1:5] %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

ville_surrey_dsm2 <- list.files(file.path("data", "Raw"), pattern = "ville_surrey_dsm", full.names = TRUE)[6:10] %>% 
  lapply(terra::rast) %>% 
  do.call(terra::merge, .)

dsm_list <- list(ville_vancouver_dsm, ville_burnaby_dsm, ville_surrey_dsm, ville_surrey_dsm2) 
dsm_list$filename <- file.path("data", "DSM_Vancouver_1m.tif")
...
aoi_mask <- terra::vect(file.path("data", "Vancouver.gpkg"))

dtm %>% 
  terra::project(terra::crs(aoi_mask)) %>% 
  crop(aoi_mask) %>% 
  mask(aoi_mask) %>% 
  terra::writeRaster(file.path("data", "DTM_Vancouver_1m.tif"), overwrite = TRUE)

dsm %>% 
  terra::project(terra::crs(aoi_mask)) %>% 
  crop(aoi_mask) %>% 
  mask(aoi_mask) %>% 
  terra::writeRaster(file.path("data", "DSM_Vancouver_1m.tif"), overwrite = TRUE)

rm(list = ls())
unlink(dir(tempdir(), pattern = ".tif", full.names = T)); invisible(gc())